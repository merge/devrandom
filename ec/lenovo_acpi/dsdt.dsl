 ...
 *
 * Disassembly of dsdt.dat, Thu Mar 14 15:14:21 2019
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00010E26 (69158)
 *     Revision         0x01 **** 32-bit table (V1), no 64-bit math support
 *     Checksum         0x50
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "TP-G2   "
 *     OEM Revision     0x00002550 (9552)
 *     Compiler ID      "INTL"
 *     Compiler Version 0x20061109 (537268489)
 */
DefinitionBlock ("", "DSDT", 1, "LENOVO", "TP-G2   ", 0x00002550)
{
            ...
            Device (LPC)
            {
                ...
                Device (EC)
                {
                    ...
                    Method (BATW, 1, NotSerialized)
                    {
                        ...
                        If (\H8DR)
                        {
                            Local1 = HB1A /* \_SB_.PCI0.LPC_.EC__.HB1A */
                        }
        ...
        Scope (\_SB.PCI0.LPC.EC)
        {
            ...
            Method (_Q45, 0, NotSerialized)  // _Qxx: EC Query
            {
                ...
                If (Local0 == 0x05)
                {
                    ...
                    {
                        \_SB.PCI0.XHCI.PR2 &= 0xFB
                    }

                    If (\_SB.PCI0.XHCI.PR3 & 0x04)
                    {
                        \_SB.PCI0.XHCI.PR3 &= 0xFB
                    }
                ...
                ElseIf (DOCD)
                {
                    ...
                    ASSI (0x02)
                    Sleep (0x64)
                    If ((\_SB.GDCK.XHOS != 0x00) && (\XHCM != 0x00))
                    {
                        If (~(\_SB.PCI0.XHCI.PR2 & 0x04))
                        {
                            \_SB.PCI0.XHCI.PR2 |= 0x04
                        }

                        If (~(\_SB.PCI0.XHCI.PR3 & 0x04))
                        {
                            \_SB.PCI0.XHCI.PR3 |= 0x04
                        }
                    }
            ...
            Method (GUSB, 1, NotSerialized)
            {
                If (\H8DR)
                {
                    ...
                    If (Arg0 == 0x00)
                    {
                        ...
                        If (\XHCM == 0x00)
                        {
                            Notify (\_SB.PCI0.EHC1.URTH.URMH.PRT2, 0x03) // Eject Request
                        }
                        ElseIf (\_SB.PCI0.XHCI.PR2 & 0x04)
                        {
                            Notify (\_SB.PCI0.XHCI.URTH.HSP2, 0x03) // Eject Request
                        }
                    }
                }
                Else
        ...
        Scope (\_SB.PCI0.LPC.EC.HKEY)
        {
            Method (GDKS, 0, NotSerialized)
            {
                ...
                If (\_SB.PCI0.LPC.EC.DOCD == 0x01)
                {
                    If (DOCD)
                    {
                        Local0 = \_SB.PCI0.LPC.EC.ACSB
                        Local0 <<= 0x01
                    }

                    Local0 |= 0x01
                    Local1 = \_SB.GDCK.GGID ()
                    Local1 <<= 0x08
                    Local0 |= Local1
                }
